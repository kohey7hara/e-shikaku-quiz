window.quizData = {
    ...
    questions: [
        ...
        { id:"audio-dft-bin", category:"DFT（計算）", question:"サンプリング周波数16kHz、DFT点数512のとき、周波数ビン間隔はいくつか。", options:["31.25Hz", "16Hz", "512Hz", "8000Hz"], answer:0, explanation:"周波数分解能は $f_s/N=16000/512=31.25$ Hzです。FFTはDFTを高速に計算するアルゴリズムで、結果の意味は同じです。" },
        { id:"audio-cepstrum-role", category:"ケプストラム", question:"スペクトルの対数をとって逆フーリエ変換するケプストラムが分離しやすくするものはどれか。", options:["声道特性と音源の周期性", "画像の前景と背景", "状態価値と行動価値", "教師データとテストデータ"], answer:0, explanation:"畳み込みとして混ざった音源と声道の影響は、対数スペクトル領域で加算になり、ケフレンシー領域で分離しやすくなります。MFCCはこの考え方を知覚尺度へ応用します。" },
        { id:"audio-wavenet-skip", category:"WaveNet（内部構造）", question:"WaveNetのResidual connectionとSkip connectionの役割の組合せとして正しいものはどれか。", options:["Residualは次層へ情報を渡し、Skipは各層の出力を最終出力側へ集約する", "両方とも未来の音を入力する", "ResidualだけがSoftmaxを行う", "Skipは勾配を遮断する"], answer:0, explanation:"各dilated blockの出力は、残差経路で深い層へ、skip経路で出力スタックへ送られます。図では2本の矢印を見分けるのがポイントです。" },
        { id:"audio-ctc-path-sum", category:"CTC（計算）", question:"ある出力文字列へ対応するCTCアラインメント経路が3本あり、各経路確率が0.10、0.05、0.02なら、その文字列の確率はいくつか。", options:["0.17", "0.001", "0.10", "0.34"], answer:0, explanation:"CTCは同じ文字列へcollapseされる全経路の確率を足します。$0.10+0.05+0.02=0.17$ です。積を取るのは各経路内の時刻確率です。" },
        { id:"audio-tts-pipeline", category:"音声合成", question:"典型的なニューラル音声合成の2段構成として正しいものはどれか。", options:["テキストからメルスペクトログラムを予測し、ボコーダで波形を生成する", "波形から画像を生成し、RPNで文字を検出する", "CTCで物体領域を抽出する", "DFTだけで文章を理解する"], answer:0, explanation:"Tacotron系などが音響特徴を生成し、WaveNetやHiFi-GAN系のニューラルボコーダが波形へ変換します。モデル名だけでなく入出力を対応づけます。" }
    ]
};
